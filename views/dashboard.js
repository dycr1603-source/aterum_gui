...
.wl-sym{font-family:var(--display);font-size:12px;font-weight:700;color:var(--text);letter-spacing:-.01em}
.wl-sym span{font-size:9px;color:var(--muted);font-weight:400}
.wl-pnl{font-family:var(--display);font-size:13px;font-weight:800;font-variant-numeric:tabular-nums;letter-spacing:-.01em}
.pp{color:var(--green)}.pn{color:var(--red)}.pz{color:var(--text2)}
.wl-row2{display:flex;justify-content:space-between;align-items:center;gap:8px}
.wl-badge{font-size:9px;font-weight:700;letter-spacing:.07em;padding:2px 7px;border-radius:999px;background:var(--bg4)}
.wl-badge.live{color:var(--green);background:rgba(0,229,160,.12)}
.wl-badge.closed{color:var(--muted);background:var(--bg4)}
.wl-r{font-size:10px;font-weight:600;font-variant-numeric:tabular-nums}
.wl-bar{height:4px;border-radius:2px;background:var(--border);overflow:hidden;margin-top:2px}
.wl-bar-fill{height:100%;border-radius:2px;transition:width .5s cubic-bezier(.4,0,.2,1)}
...
/* ========================================================================== */
/* ATERUM INSTITUTIONAL TERMINAL — visual layer only                           */
/* Scoped to dashboard-v3 so data, chart and shared application flows remain  */
/* untouched. All dimensions and colors consume the shared --at-* tokens.     */
/* ========================================================================== */
body.dashboard-v3{
  --terminal-header:60px;
  background:
    radial-gradient(circle at 12% -12%,rgba(53,109,255,.13),transparent 29%),
    radial-gradient(circle at 84% 4%,rgba(155,77,255,.09),transparent 24%),
    linear-gradient(180deg,var(--at-canvas),#02060e 52%,#01050b) !important;
  color:var(--at-text-strong) !important;
  font-family:var(--at-font-ui);
}
body.dashboard-v3 .app-chrome{
  display:block !important;
  opacity:.42;
}
body.dashboard-v3 .app-grid{
  display:block !important;
  background-size:72px 72px;
  opacity:.08;
  mask-image:linear-gradient(180deg,#000,transparent 72%);
}
body.dashboard-v3 .app-grid-secondary,
body.dashboard-v3 .app-scanlines,
body.dashboard-v3 .app-pixel-lane,
body.dashboard-v3 .app-particles{
  display:none !important;
}
body.dashboard-v3 .app-vignette{
  display:block !important;
  background:radial-gradient(circle at center,transparent 46%,rgba(0,2,7,.4));
}
...
/* Header */
body.dashboard-v3 .nav-shell{
  top:0;
  min-height:var(--terminal-header);
  ...
  box-shadow:0 8px 30px rgba(0,0,0,.2),inset 0 -1px rgba(78,167,255,.035);
  backdrop-filter:blur(20px) saturate(125%);
}
body.dashboard-v3 .nav-main{gap:18px}
body.dashboard-v3 .nav-logo{
  min-height:42px;
...
body.dashboard-v3 .nav-dot{
  width:16px;
  height:24px;
  border-radius:2px;
  background:linear-gradient(160deg,var(--at-cyan),var(--at-blue) 48%,var(--at-violet));
  box-shadow:0 0 18px rgba(78,167,255,.38);
  clip-path:polygon(50% 0,100% 100%,70% 100%,50% 58%,30% 100%,0 100%);
}
body.dashboard-v3 .nav-links-desktop{gap:2px}
body.dashboard-v3 .nav-link{
  ...
  font-size:9px;
  letter-spacing:.08em;
  transition:color var(--at-motion-fast) ease-out,opacity var(--at-motion-fast) ease-out,transform var(--at-motion-base) ease-out;
}
body.dashboard-v3 .nav-link:hover{
  border-color:transparent;
  background:rgba(78,167,255,.055);
  color:var(--at-text-strong);
  transform:translateY(-1px) !important;
}
body.dashboard-v3 .nav-link.active{
  border-color:rgba(78,167,255,.12);
  background:linear-gradient(180deg,rgba(78,167,255,.105),rgba(53,109,255,.045));
  box-shadow:0 8px 24px rgba(27,92,255,.09),inset 0 1px rgba(255,255,255,.025);
}
body.dashboard-v3 .nav-link.active::after{
  left:14px;
  right:14px;
  bottom:-1px;
  height:2px;
  background:var(--at-cyan);
  box-shadow:0 0 12px rgba(78,167,255,.7);
}
body.dashboard-v3 .nav-utility{gap:8px}
body.dashboard-v3 .nav-market{
  min-height:38px;
  padding:0 12px;
  gap:10px;
  border-color:var(--at-line);
...
body.dashboard-v3 .nav-sym,
body.dashboard-v3 .nav-time,
body.dashboard-v3 .nav-live,
body.dashboard-v3 .nav-pill{
  font-family:var(--at-font-mono);
  font-size:9px;
  letter-spacing:.04em;
}
body.dashboard-v3 .nav-price{
  min-width:auto;
  font-family:var(--at-font-display);
  font-size:18px;
  font-weight:750;
...
body.dashboard-v3 .theme-toggle,
body.dashboard-v3 .nav-user,
body.dashboard-v3 .nav-link-ghost{
  min-height:36px;
  border-color:var(--at-line);
  background:rgba(7,15,29,.72);
  border-radius:var(--at-radius-control);
}
body.dashboard-v3 .theme-toggle-label{display:none}

/* Three-column terminal */
body.dashboard-v3 .layout{
  display:grid !important;
  grid-template-columns:minmax(226px,244px) minmax(0,1fr) minmax(268px,286px);
  align-items:start;
  gap:12px;
  width:100%;
  height:auto !important;
  min-height:calc(100dvh - var(--terminal-header));
  padding:12px !important;
}
body.dashboard-v3 .layout > *{min-width:0}
body.dashboard-v3 .wl,
body.dashboard-v3 .sb{
  position:sticky;
  top:calc(var(--terminal-header) + 12px);
  width:auto !important;
  min-width:0 !important;
  max-width:none !important;
  height:calc(100dvh - var(--terminal-header) - 24px) !important;
  max-height:calc(100dvh - var(--terminal-header) - 24px) !important;
  padding:0 !important;
  gap:10px;
  overflow:hidden;
  border:1px solid var(--at-line) !important;
  border-radius:var(--at-radius-shell) !important;
  background:linear-gradient(180deg,rgba(7,15,29,.91),rgba(4,10,20,.94)) !important;
  box-shadow:var(--at-shadow-panel) !important;
  backdrop-filter:blur(18px) saturate(115%);
}
body.dashboard-v3 .wl{display:flex;flex-direction:column}
body.dashboard-v3 .sb{
  display:flex;
  flex-direction:column;
  overflow-y:auto;
  scrollbar-width:thin;
  scrollbar-color:rgba(78,167,255,.25) transparent;
}
...
body.dashboard-v3 .wl::-webkit-scrollbar,
body.dashboard-v3 .sb::-webkit-scrollbar,
body.dashboard-v3 .wl-scroll::-webkit-scrollbar{width:5px}
body.dashboard-v3 .wl::-webkit-scrollbar-thumb,
body.dashboard-v3 .sb::-webkit-scrollbar-thumb,
body.dashboard-v3 .wl-scroll::-webkit-scrollbar-thumb{
  border-radius:999px;
  background:rgba(78,167,255,.22);
}

/* Unified surface language */
body.dashboard-v3 :is(.side-card,.terminal-card,.chart-shell,.metric-card,.market-card,.wave-card,.stat-c,.lv,.result-row){
  border:1px solid var(--at-line) !important;
  background:var(--at-gradient-panel) !important;
  background-image:var(--at-gradient-panel) !important;
  box-shadow:var(--at-shadow-panel) !important;
  transition:transform var(--at-motion-base) ease-out,opacity var(--at-motion-fast) ease-out,box-shadow var(--at-motion-slow) ease-out,border-color var(--at-motion-base) ease-out;
}
body.dashboard-v3 :is(.side-card,.terminal-card,.metric-card,.market-card,.wave-card,.stat-c,.lv,.result-row):hover{
  transform:translateY(-1px) !important;
  border-color:var(--at-line-strong) !important;
  box-shadow:var(--at-shadow-hover) !important;
}
body.dashboard-v3 :is(.side-card-label,.terminal-card-title,.chart-eyebrow,.metric-label,.market-label,.wave-label,.wl-hdr,.sec-ttl,.stat-lbl,.pm-lbl,.panel-label,.workspace-stat-label){
...
body.dashboard-v3 .side-card{
  border-radius:var(--at-radius-card) !important;
  overflow:hidden;
}
body.dashboard-v3 .workspace-card{
  flex:none;
  padding:16px;
  border-width:0 0 1px;
  border-radius:var(--at-radius-shell) var(--at-radius-shell) 0 0 !important;
  background:
    radial-gradient(circle at 8% 0,rgba(78,167,255,.11),transparent 34%),
    linear-gradient(155deg,rgba(12,25,47,.92),rgba(5,12,24,.96));
}
body.dashboard-v3 .workspace-user{
  margin-top:12px;
  color:var(--at-text-strong);
  font-family:var(--at-font-display);
  font-size:15px;
...
body.dashboard-v3 .workspace-sub{
  margin-top:8px;
  color:var(--at-text-soft);
  font-size:10px;
  line-height:1.6;
}
body.dashboard-v3 .workspace-sub span{display:block;color:var(--at-text-strong);font-family:var(--at-font-mono)}
body.dashboard-v3 .workspace-grid{
  gap:0;
  margin-top:12px;
  padding-top:10px;
  border-top:1px solid var(--at-line);
}
body.dashboard-v3 .workspace-stat{padding:9px 8px 7px 0}
body.dashboard-v3 .workspace-stat-value{
  margin-top:5px;
  color:var(--at-text-strong);
  font-family:var(--at-font-mono);
  font-size:12px;
...
body.dashboard-v3 .workspace-progress{margin-top:10px}
body.dashboard-v3 .workspace-progress-top{font-family:var(--at-font-mono);font-size:8px;color:var(--at-text-soft)}
body.dashboard-v3 .workspace-bar{height:4px;margin-top:8px;background:rgba(87,110,148,.14)}
body.dashboard-v3 .workspace-bar-fill{background:linear-gradient(90deg,var(--at-teal),var(--at-cyan));box-shadow:0 0 10px rgba(32,227,178,.3)}
body.dashboard-v3 .wl-hdr{
  flex:none;
  padding:4px 14px 0 !important;
  border:0 !important;
}
body.dashboard-v3 .wl-count{
...
body.dashboard-v3 .wl-sec-lbl{font-family:var(--at-font-mono);font-size:8px;color:var(--at-text-faint)}
body.dashboard-v3 .wl-item{
  ...
  transition:transform var(--at-motion-fast) ease-out,background var(--at-motion-fast) ease-out,border-color var(--at-motion-fast) ease-out;
}
body.dashboard-v3 .wl-item:hover{
  transform:translateX(2px) !important;
  border-color:var(--at-line);
  background:rgba(15,29,53,.72);
}
body.dashboard-v3 .wl-item.active{
  border-color:rgba(78,167,255,.2);
  background:linear-gradient(90deg,rgba(53,109,255,.12),rgba(12,24,43,.62));
  box-shadow:inset 2px 0 var(--at-cyan),0 10px 24px rgba(0,0,0,.16);
}
body.dashboard-v3 .wl-sym{font-family:var(--at-font-display);font-size:11px}
body.dashboard-v3 .wl-pnl{font-family:var(--at-font-mono);font-size:10px}
body.dashboard-v3 .wl-badge,body.dashboard-v3 .wl-r{font-family:var(--at-font-mono);font-size:7px}
body.dashboard-v3 .telemetry-card{
  flex:none;
  margin:0;
  padding:13px 14px;
  border-width:1px 0 0;
  border-radius:0 0 var(--at-radius-shell) var(--at-radius-shell) !important;
  background:rgba(6,14,27,.76);
}
body.dashboard-v3 .telemetry-row{min-height:26px;border-color:rgba(113,151,214,.09);font-size:9px}
body.dashboard-v3 .telemetry-row strong{font-family:var(--at-font-mono);font-size:9px}
...
body.dashboard-v3 .metric-card::before{
  display:block !important;
  background:radial-gradient(circle at 0 0,rgba(78,167,255,.08),transparent 50%);
  opacity:1;
}
...
body.dashboard-v3 .metric-card-top{margin:0;min-height:28px}
body.dashboard-v3 .metric-icon{
  width:30px;
  height:30px;
  border:1px solid currentColor;
  border-radius:9px;
  box-shadow:0 0 18px color-mix(in srgb,currentColor 14%,transparent);
}
body.dashboard-v3 .metric-icon svg{width:14px;height:14px}
body.dashboard-v3 .metric-spark{position:absolute;right:8px;bottom:7px;width:50%;height:30px;opacity:.78}
body.dashboard-v3 .metric-label{margin-top:7px}
...
body.dashboard-v3 .metric-sub{position:relative;z-index:1;margin-top:6px;color:var(--at-text-soft);font-size:9px}
body.dashboard-v3 .metric-card-brand{
  background:
    linear-gradient(145deg,rgba(72,28,137,.18),rgba(5,14,30,.9)),
    radial-gradient(circle at 50% 42%,rgba(155,77,255,.2),transparent 58%) !important;
}
body.dashboard-v3 .brand-prism{
  width:52px;
  animation:dashboardPrismFloat 5s ease-in-out infinite;
  filter:drop-shadow(0 0 12px rgba(155,77,255,.42)) drop-shadow(0 0 24px rgba(53,109,255,.24));
}
@keyframes dashboardPrismFloat{
  0%,100%{transform:translateY(0);opacity:.9}
  50%{transform:translateY(-3px);opacity:1}
}

/* Chart shell — canvas implementation remains untouched */
body.dashboard-v3 .chart-shell{
  gap:8px;
  padding:13px !important;
  border-radius:var(--at-radius-shell) !important;
  background:
    radial-gradient(circle at 20% 0,rgba(53,109,255,.055),transparent 31%),
    linear-gradient(180deg,rgba(7,15,29,.96),rgba(3,9,19,.98)) !important;
  background-image:
    radial-gradient(circle at 20% 0,rgba(53,109,255,.055),transparent 31%),
    linear-gradient(180deg,rgba(7,15,29,.96),rgba(3,9,19,.98)) !important;
  box-shadow:var(--at-shadow-panel) !important;
...
body.dashboard-v3 .chart-shell::before{display:block !important;opacity:.34}
body.dashboard-v3 .chart-head{padding:1px 3px 5px}
...
body.dashboard-v3 .chart-sub{margin-top:6px;color:var(--at-text-soft);font-size:9px}
body.dashboard-v3 .chart-head-side{gap:8px}
...
body.dashboard-v3 .chart-structure strong{margin-top:4px;color:var(--at-violet);font-family:var(--at-font-display);font-size:12px}
body.dashboard-v3 .chart-fs-btn{
  ...
  font-family:var(--at-font-mono);
  font-size:8px;
  transition:transform var(--at-motion-fast) ease-out,opacity var(--at-motion-fast) ease-out;
}
body.dashboard-v3 .chart-fs-btn:hover{transform:translateY(-1px) !important}
...
body.dashboard-v3 .ind-btn{
  ...
  background:rgba(13,25,45,.58) !important;
  font-family:var(--at-font-mono);
  font-size:8px;
  letter-spacing:.05em;
  transition:transform var(--at-motion-fast) ease-out,opacity var(--at-motion-fast) ease-out;
}
body.dashboard-v3 .ind-btn:hover{transform:translateY(-1px) !important}
body.dashboard-v3 .ind-sep{background:var(--at-line)}
body.dashboard-v3 #chart-container{
  height:clamp(430px,51vh,620px);
  min-height:430px;
  border:1px solid rgba(113,151,214,.11) !important;
  border-radius:10px;
  background:linear-gradient(180deg,rgba(2,8,17,.92),rgba(2,7,15,.98)) !important;
  box-shadow:inset 0 0 0 1px rgba(78,167,255,.02),inset 0 18px 42px rgba(4,13,27,.2);
}
body.dashboard-v3 #chart-container:hover{
  box-shadow:inset 0 0 0 1px rgba(78,167,255,.07),inset 0 18px 42px rgba(4,13,27,.2);
}
body.dashboard-v3 .sub-panel{border-top-color:var(--at-line) !important;background:rgba(3,9,18,.96) !important}
...
body.dashboard-v3 .market-sub{position:relative;z-index:1;margin-top:7px;color:var(--at-text-soft);font-size:8px}
body.dashboard-v3 .market-spark{right:4px;bottom:4px;width:62%;height:28px;opacity:.48}
body.dashboard-v3 .sentiment-card{gap:2px;padding:9px}
body.dashboard-v3 .sentiment-gauge{width:52px;height:52px}
body.dashboard-v3 .sentiment-value{margin-top:-34px;font-size:17px}
body.dashboard-v3 .sentiment-label-sub{margin-top:15px;font-family:var(--at-font-mono);font-size:7px}
body.dashboard-v3 .wave-row{gap:8px;margin-top:0}
body.dashboard-v3 .wave-card{min-height:84px;padding:12px 14px;border-radius:var(--at-radius-card)}
body.dashboard-v3 .wave-value{font-family:var(--at-font-display);font-size:17px}
body.dashboard-v3 .wave-sub{font-size:8px;color:var(--at-text-soft)}
body.dashboard-v3 .wave-spark{position:absolute;right:8px;bottom:6px;width:58%;height:36px;opacity:.64}
...
body.dashboard-v3 .terminal-card:first-child{border-radius:var(--at-radius-shell) var(--at-radius-shell) 0 0 !important}
body.dashboard-v3 .details-card{border-bottom:0}
body.dashboard-v3 .terminal-card-hdr{
  min-height:42px;
  padding:12px 14px;
  border-bottom:1px solid var(--at-line);
  background:rgba(9,19,36,.52) !important;
}
body.dashboard-v3 .terminal-card-meta{color:var(--at-text-faint);font-family:var(--at-font-mono);font-size:7px}
body.dashboard-v3 .exec-pill{
  padding:4px 8px;
  border-color:rgba(32,227,178,.16);
  border-radius:var(--at-radius-pill);
...
body.dashboard-v3 .exec-body{padding:12px 14px 14px;gap:10px}
body.dashboard-v3 .exec-toggle,body.dashboard-v3 .exec-mode{gap:6px}
body.dashboard-v3 .exec-btn,
body.dashboard-v3 .exec-mode-btn{
  min-height:36px;
  border-color:var(--at-line) !important;
  border-radius:8px !important;
  background:rgba(12,24,44,.72) !important;
  color:var(--at-text-soft);
  font-family:var(--at-font-mono);
  font-size:8px;
  transition:transform var(--at-motion-fast) ease-out,opacity var(--at-motion-fast) ease-out;
}
body.dashboard-v3 .exec-btn:hover,body.dashboard-v3 .exec-mode-btn:hover{transform:translateY(-1px) !important}
body.dashboard-v3 .exec-btn.active{
  border-color:rgba(32,227,178,.5);
  background:linear-gradient(180deg,rgba(32,227,178,.22),rgba(16,160,128,.13)) !important;
  color:#d9fff5;
  box-shadow:0 0 18px rgba(32,227,178,.11),inset 0 1px rgba(255,255,255,.06);
}
body.dashboard-v3 .exec-slider-group{gap:7px}
body.dashboard-v3 .exec-slider-label{font-family:var(--at-font-mono);font-size:7px;color:var(--at-text-soft)}
body.dashboard-v3 .exec-slider{height:4px;background:rgba(87,110,148,.14)}
body.dashboard-v3 .exec-slider-fill{background:linear-gradient(90deg,var(--at-blue),var(--at-cyan));box-shadow:0 0 9px rgba(78,167,255,.28)}
body.dashboard-v3 .exec-symbol-row{padding-top:9px;border-top:1px solid var(--at-line)}
body.dashboard-v3 .exec-symbol{font-family:var(--at-font-display);font-size:22px !important;font-weight:740;letter-spacing:-.04em}
body.dashboard-v3 .exec-side{padding:4px 8px;border-radius:var(--at-radius-pill);font-family:var(--at-font-mono);font-size:7px}
body.dashboard-v3 .exec-exposure{
  padding:11px 12px;
  border-color:rgba(245,185,77,.12);
  border-radius:10px;
  background:linear-gradient(145deg,rgba(38,29,14,.45),rgba(8,15,28,.82));
  box-shadow:inset 0 1px rgba(255,255,255,.025);
}
body.dashboard-v3 .exec-exposure-label{font-family:var(--at-font-mono);font-size:7px;color:var(--at-text-soft)}
body.dashboard-v3 .exec-exposure-value{margin-top:7px;font-size:22px}
body.dashboard-v3 .exec-exposure-chip{padding:3px 7px;font-family:var(--at-font-mono);font-size:6px}
body.dashboard-v3 .exec-exposure-meta{font-family:var(--at-font-mono);font-size:11px}
body.dashboard-v3 .exec-pnl-panel{
  padding:12px;
  border-color:var(--at-line);
  border-radius:10px;
...
body.dashboard-v3 .exec-pnl-top,body.dashboard-v3 .exec-pnl-sub{font-family:var(--at-font-mono);font-size:7px;color:var(--at-text-soft)}
body.dashboard-v3 .exec-pnl-value{margin-top:7px;font-family:var(--at-font-display);font-size:29px}
body.dashboard-v3 .pnl-hero{padding:14px}
body.dashboard-v3 .pnl-lbl{font-family:var(--at-font-mono);font-size:8px}
body.dashboard-v3 .pnl-val{font-family:var(--at-font-display);font-size:32px !important;letter-spacing:-.05em}
body.dashboard-v3 .pnl-status-badge{font-family:var(--at-font-mono);font-size:7px}
body.dashboard-v3 .pnl-meta{gap:8px;margin-top:11px}
body.dashboard-v3 .pm-val{font-family:var(--at-font-mono);font-size:10px}
body.dashboard-v3 .sec{padding:12px 14px;border-color:var(--at-line)}
body.dashboard-v3 .sec-ttl{margin-bottom:8px}
body.dashboard-v3 .levels{gap:5px}
body.dashboard-v3 .lv{padding:9px 10px;border-radius:8px;background:rgba(10,21,39,.58);box-shadow:none}
body.dashboard-v3 .lv-name,body.dashboard-v3 .lv-dist{font-size:8px}
body.dashboard-v3 .lv-price{font-family:var(--at-font-mono);font-size:11px}
body.dashboard-v3 .sl-indicator,body.dashboard-v3 .tp-indicator{padding:6px 8px;border-radius:7px;font-family:var(--at-font-mono);font-size:7px}
body.dashboard-v3 .r-wrap{gap:8px}
body.dashboard-v3 .r-big{font-family:var(--at-font-display);font-size:30px}
body.dashboard-v3 .r-track{height:5px;background:rgba(87,110,148,.14)}
body.dashboard-v3 .r-mark{font-family:var(--at-font-mono);font-size:6px;color:var(--at-text-faint)}
body.dashboard-v3 .stats-grid{gap:6px}
body.dashboard-v3 .stat-c{min-height:58px;padding:9px 10px;border-radius:8px;background:rgba(10,21,39,.58);box-shadow:none}
body.dashboard-v3 .stat-val{font-family:var(--at-font-mono);font-size:12px !important}
body.dashboard-v3 .score-gauge{width:24px;height:24px}

/* Fullscreen and responsive behavior retain the existing state classes. */
body.dashboard-v3 #chart-wrap.is-fullscreen{padding:12px;background:var(--at-canvas) !important}
body.dashboard-v3 #chart-wrap.is-fullscreen .chart-shell{border-radius:var(--at-radius-shell)}
@media (max-width:1480px){
  body.dashboard-v3 .layout{grid-template-columns:218px minmax(0,1fr) 264px;gap:10px;padding:10px !important}
  body.dashboard-v3 .terminal-kpis{grid-template-columns:repeat(4,minmax(0,1fr)) 92px}
  body.dashboard-v3 .metric-card{padding-inline:12px}
  body.dashboard-v3 .metric-value{font-size:22px}
  body.dashboard-v3 .market-grid{grid-template-columns:repeat(4,minmax(0,1fr)) 96px}
  body.dashboard-v3 .market-value{font-size:19px}
  body.dashboard-v3 .nav-link{padding-inline:8px;font-size:8px}
}
@media (max-width:1220px){
  body.dashboard-v3 .layout{grid-template-columns:204px minmax(0,1fr) 246px;gap:8px;padding:8px !important}
  body.dashboard-v3 .terminal-kpis{grid-template-columns:repeat(4,minmax(0,1fr))}
  body.dashboard-v3 .metric-card-brand{display:none}
  body.dashboard-v3 .market-grid{grid-template-columns:repeat(4,minmax(0,1fr))}
  body.dashboard-v3 .sentiment-card{display:none}
  body.dashboard-v3 .nav-main{gap:8px}
  body.dashboard-v3 .nav-logo{padding-right:6px;font-size:12px}
  body.dashboard-v3 .nav-links-desktop{gap:0}
  body.dashboard-v3 .nav-link{padding-inline:6px;font-size:7px}
  body.dashboard-v3 .nav-market{padding-inline:9px;gap:7px}
  body.dashboard-v3 .theme-toggle,body.dashboard-v3 .nav-user,body.dashboard-v3 .nav-link-ghost{padding-inline:8px}
}
@media (max-width:1024px){
  body.dashboard-v3 .layout{
    grid-template-columns:minmax(0,1fr) 250px;
    grid-template-areas:'chart stats' 'watch stats';
    align-items:start;
    gap:10px;
    padding:10px !important;
  }
  body.dashboard-v3 #chart-wrap{grid-area:chart;padding:0}
  body.dashboard-v3 .sb{grid-area:stats;position:sticky;top:calc(var(--terminal-header) + 10px);height:calc(100dvh - var(--terminal-header) - 20px) !important;max-height:calc(100dvh - var(--terminal-header) - 20px) !important}
  body.dashboard-v3 .wl{
    grid-area:watch;
    position:relative;
    top:auto;
  ...
  body.dashboard-v3 .workspace-card,body.dashboard-v3 .telemetry-card{display:none}
  body.dashboard-v3 .wl-scroll{max-height:230px}
  body.dashboard-v3 #chart-container{height:clamp(390px,50vh,540px);min-height:390px}
  body.dashboard-v3 .wave-row{grid-template-columns:1fr}
  body.dashboard-v3 .wave-card{min-height:72px}
}
@media (max-width:840px){
  body.dashboard-v3{--terminal-header:58px}
  body.dashboard-v3 .nav-shell{top:0;margin:0;min-height:var(--terminal-header);height:var(--terminal-header);padding-inline:10px;border-radius:0 !important}
  body.dashboard-v3 .layout{display:block !important;padding:10px 10px 76px !important}
  body.dashboard-v3 #chart-wrap{display:flex}
  body.dashboard-v3 .wl,body.dashboard-v3 .sb{
    position:fixed;
    top:var(--terminal-header);
    bottom:58px;
    z-index:160;
  ...
  body.dashboard-v3 .wl{left:0;transform:translateX(-104%)}
  body.dashboard-v3 .sb{right:0;transform:translateX(104%)}
  body.dashboard-v3 .wl.open,body.dashboard-v3 .sb.open{transform:translateX(0)}
  body.dashboard-v3 .workspace-card,body.dashboard-v3 .telemetry-card{display:block}
  body.dashboard-v3 .terminal-kpis{grid-template-columns:repeat(2,minmax(0,1fr))}
  body.dashboard-v3 .metric-card{min-height:104px}
  body.dashboard-v3 .market-grid{grid-template-columns:repeat(2,minmax(0,1fr))}
  body.dashboard-v3 .sentiment-card{display:flex}
  body.dashboard-v3 #chart-container{height:48vh;min-height:340px}
  body.dashboard-v3 .chart-title{font-size:25px}
  body.dashboard-v3 .chart-head{flex-direction:row;align-items:center}
  body.dashboard-v3 .chart-sub,body.dashboard-v3 .chart-structure,body.dashboard-v3 .chart-fs-text,body.dashboard-v3 .chart-fs-key{display:none}
  body.dashboard-v3 .wave-row{grid-template-columns:1fr}
}
@media (max-width:560px){
  body.dashboard-v3 .layout{padding-inline:7px !important}
  body.dashboard-v3 .terminal-kpis,body.dashboard-v3 .market-grid{grid-template-columns:1fr 1fr;gap:6px}
  body.dashboard-v3 .metric-card{padding:12px;min-height:100px}
  body.dashboard-v3 .metric-value{font-size:21px}
  body.dashboard-v3 .metric-sub{font-size:8px}
  body.dashboard-v3 .market-card{min-height:82px}
  body.dashboard-v3 .market-value{font-size:18px}
  body.dashboard-v3 .chart-shell{padding:8px}
  body.dashboard-v3 .chart-head-side{margin-left:auto}
  body.dashboard-v3 #chart-container{min-height:320px}
  body.dashboard-v3 .ind-toolbar{overflow-x:auto;scrollbar-width:none}
  body.dashboard-v3 .ind-toolbar::-webkit-scrollbar{display:none}
}

/* ========================================================================== */
/* PREMIUM DEPTH PASS — ambient canvas, layered glass and GPU-only motion      */
/* ========================================================================== */
body.dashboard-v3{
  --terminal-header:64px;
  isolation:isolate;
  background:
    radial-gradient(ellipse 72% 48% at 52% -18%,rgba(64,75,255,.28),transparent 68%),
    radial-gradient(ellipse 38% 42% at -8% 34%,rgba(32,185,255,.14),transparent 72%),
    radial-gradient(ellipse 42% 48% at 108% 52%,rgba(139,61,255,.16),transparent 74%),
    linear-gradient(145deg,#020714 0%,#040817 34%,#020610 72%,#01040b 100%) !important;
}
body.dashboard-v3 .app-chrome{
  display:block !important;
  opacity:1;
  background:
    radial-gradient(circle at 22% 16%,rgba(65,111,255,.13),transparent 22%),
    radial-gradient(circle at 78% 22%,rgba(148,70,255,.095),transparent 24%),
    radial-gradient(circle at 52% 78%,rgba(30,196,219,.055),transparent 30%);
}
body.dashboard-v3 .app-chrome::before{
  content:'';
  position:absolute;
  inset:-12%;
  background:
    radial-gradient(circle at 24% 32%,rgba(68,123,255,.12),transparent 19%),
    radial-gradient(circle at 72% 42%,rgba(139,71,255,.1),transparent 22%);
  opacity:.78;
  transform:translate3d(0,0,0) scale(1);
  animation:terminalAmbientDrift 18s ease-in-out infinite alternate;
}
body.dashboard-v3 .app-chrome::after{
  content:'';
  position:absolute;
  inset:0;
  opacity:.13;
...
body.dashboard-v3 .app-grid{
  ...
  opacity:.14;
  mask-image:linear-gradient(180deg,#000 0%,rgba(0,0,0,.72) 48%,transparent 94%);
}
body.dashboard-v3 .app-vignette{
  display:block !important;
  background:
    radial-gradient(ellipse at center,transparent 38%,rgba(0,2,10,.32) 78%,rgba(0,1,7,.68) 100%),
    linear-gradient(180deg,rgba(4,8,24,.04),rgba(0,2,8,.34));
}
@keyframes terminalAmbientDrift{
  0%{transform:translate3d(-2%,-1%,0) scale(1);opacity:.62}
  100%{transform:translate3d(3%,2%,0) scale(1.06);opacity:.88}
}

/* Floating institutional command bar */
body.dashboard-v3 .nav-shell{
  top:8px;
  width:calc(100% - 24px);
  min-height:56px;
  ...
  margin:8px 12px 0;
  padding:0 13px;
  border:1px solid rgba(117,157,225,.19);
  border-radius:14px !important;
  background:
    linear-gradient(180deg,rgba(12,21,40,.82),rgba(5,11,24,.76)),
    radial-gradient(circle at 18% 0,rgba(78,167,255,.08),transparent 34%);
  box-shadow:var(--at-shadow-float);
  backdrop-filter:blur(var(--at-blur-float)) saturate(145%);
  overflow:hidden;
}
body.dashboard-v3 .nav-shell::before{
  content:'';
  position:absolute;
  z-index:0;
  left:2%;
...
body.dashboard-v3 .nav-shell::after{
  content:'';
  position:absolute;
  z-index:0;
  top:-80%;
  ...
  left:-24%;
  width:20%;
  background:linear-gradient(90deg,transparent,rgba(166,201,255,.075),transparent);
  transform:translate3d(0,0,0) skewX(-18deg);
  animation:navLightPass 9s ease-in-out infinite;
}
body.dashboard-v3 .nav-shell > *{position:relative;z-index:1}
@keyframes navLightPass{
  0%,62%{transform:translate3d(0,0,0) skewX(-18deg);opacity:0}
  70%{opacity:1}
  100%{transform:translate3d(720%,0,0) skewX(-18deg);opacity:0}
}
body.dashboard-v3 .nav-logo{
...
body.dashboard-v3 .nav-link{
  opacity:.72;
  transition:transform var(--at-motion-base) ease-out,opacity var(--at-motion-fast) ease-out !important;
}
body.dashboard-v3 .nav-link:hover,
body.dashboard-v3 .nav-link.active{opacity:1}
body.dashboard-v3 .nav-link.active{
  background:linear-gradient(180deg,rgba(66,132,255,.18),rgba(45,80,205,.08));
  box-shadow:inset 0 1px rgba(255,255,255,.06),0 10px 28px rgba(34,94,255,.13);
}
body.dashboard-v3 .nav-market{
  border-color:rgba(107,150,224,.19);
  background:linear-gradient(180deg,rgba(8,17,34,.88),rgba(4,10,22,.74));
  box-shadow:inset 0 1px rgba(255,255,255,.04),0 12px 28px rgba(0,2,10,.28);
}
body.dashboard-v3 .nav-price{
  background:linear-gradient(110deg,#4df5ce,#55b9ff 60%,#9d74ff);
  background-clip:text;
  -webkit-background-clip:text;
  color:transparent !important;
  text-shadow:0 0 24px rgba(62,212,213,.2);
}
body.dashboard-v3 .live-dot{
  box-shadow:0 0 0 4px rgba(32,227,178,.075),0 0 18px rgba(32,227,178,.65);
  animation:terminalLivePulse 2.4s ease-in-out infinite;
}
...
body.dashboard-v3 .nav-user,
body.dashboard-v3 .nav-link-ghost{
  box-shadow:inset 0 1px rgba(255,255,255,.035),0 8px 20px rgba(0,2,10,.18);
}

/* Canvas spacing and independent elevation zones */
body.dashboard-v3 .layout{
  gap:14px;
...
body.dashboard-v3 .sb{
  ...
  border-color:rgba(107,147,216,.18) !important;
  background:
    linear-gradient(168deg,rgba(12,23,43,.82),rgba(4,10,22,.92) 54%,rgba(7,12,27,.88)),
...
body.dashboard-v3 .sb::before{
  ...
  border-radius:inherit;
  background:
    linear-gradient(105deg,rgba(255,255,255,.045),transparent 26%),
    radial-gradient(circle at 50% 0,rgba(91,132,255,.13),transparent 28%);
  opacity:.65;
  pointer-events:none;
}
body.dashboard-v3 .wl::after,
body.dashboard-v3 .sb::after{
  content:'';
  position:absolute;
  z-index:5;
  top:0;
...
body.dashboard-v3 .wl > *,body.dashboard-v3 .sb > *{position:relative;z-index:6}
body.dashboard-v3 #chart-wrap{gap:14px}

/* Workspace and watchlist */
body.dashboard-v3 .workspace-card{
  margin:8px 8px 0;
  padding:17px;
  border:1px solid rgba(103,151,229,.17) !important;
  border-radius:12px !important;
  background:
    radial-gradient(circle at 8% 0,rgba(70,148,255,.2),transparent 35%),
    radial-gradient(circle at 92% 16%,rgba(128,70,255,.13),transparent 34%),
    linear-gradient(150deg,rgba(17,33,61,.92),rgba(7,15,31,.94)) !important;
  box-shadow:var(--at-shadow-z2) !important;
  overflow:hidden;
}
body.dashboard-v3 .workspace-card::before{
  content:'';
  position:absolute;
  inset:0;
  background:var(--at-glass-sheen);
  opacity:.55;
  pointer-events:none;
}
...
body.dashboard-v3 .r-track{
  box-shadow:inset 0 1px 3px rgba(0,0,0,.48),0 1px rgba(255,255,255,.025);
}
body.dashboard-v3 .workspace-bar-fill,
body.dashboard-v3 .exec-slider-fill{
  position:relative;
  box-shadow:0 0 14px rgba(53,176,255,.38);
}
body.dashboard-v3 .wl-hdr{padding:7px 14px 1px !important}
body.dashboard-v3 .wl-count{
  box-shadow:0 0 18px rgba(78,167,255,.12),inset 0 1px rgba(255,255,255,.04);
}
body.dashboard-v3 .wl-scroll{
  padding:8px !important;
...
body.dashboard-v3 .wl-item{
  position:relative;
  overflow:hidden;
  border-color:rgba(105,145,214,.085);
  background:
    linear-gradient(135deg,rgba(18,33,57,.7),rgba(6,14,28,.68)) !important;
  box-shadow:inset 0 1px rgba(255,255,255,.025),0 8px 20px rgba(0,2,10,.16) !important;
  transition:transform var(--at-motion-base) ease-out,opacity var(--at-motion-fast) ease-out !important;
}
body.dashboard-v3 .wl-item::before{
  content:'';
  position:absolute;
  inset:-30%;
  background:radial-gradient(circle at 0 50%,rgba(61,145,255,.22),transparent 44%);
  opacity:0;
  transform:scale(.88);
  transition:transform var(--at-motion-slow) ease-out,opacity var(--at-motion-base) ease-out;
  pointer-events:none;
}
body.dashboard-v3 .wl-item:hover::before,
body.dashboard-v3 .wl-item.active::before{opacity:1;transform:scale(1)}
body.dashboard-v3 .wl-item:hover{transform:translate3d(3px,-1px,0) scale(1.008) !important}
body.dashboard-v3 .wl-item.active{
  border-color:rgba(82,160,255,.3) !important;
  box-shadow:var(--at-glow-blue) !important;
}
body.dashboard-v3 .wl-item > *{position:relative;z-index:1}
body.dashboard-v3 .telemetry-card{
  margin:0 8px 8px;
  border:1px solid rgba(103,151,229,.12) !important;
  border-radius:12px !important;
  background:linear-gradient(145deg,rgba(13,26,49,.8),rgba(5,12,25,.84)) !important;
  box-shadow:var(--at-shadow-z1) !important;
}
...
body.dashboard-v3 .metric-card{
  ...
  min-height:122px;
  border-color:rgba(var(--metric-accent),.16) !important;
  background:
    radial-gradient(circle at 8% 0,rgba(var(--metric-accent),.16),transparent 38%),
    radial-gradient(circle at 94% 106%,rgba(var(--metric-accent),.09),transparent 42%),
    linear-gradient(145deg,rgba(15,29,53,.91),rgba(5,12,26,.93)) !important;
  box-shadow:var(--at-shadow-z2) !important;
  transform:translate3d(0,0,0);
  transition:transform var(--at-motion-base) ease-out,opacity var(--at-motion-fast) ease-out !important;
  animation:premiumCardEnter .5s var(--at-ease) both;
}
body.dashboard-v3 .metric-card:nth-child(1){--metric-accent:32,227,178;animation-delay:40ms}
body.dashboard-v3 .metric-card:nth-child(2){--metric-accent:78,167,255;animation-delay:80ms}
body.dashboard-v3 .metric-card:nth-child(3){--metric-accent:245,185,77;animation-delay:120ms}
body.dashboard-v3 .metric-card:nth-child(4){--metric-accent:255,79,109;animation-delay:160ms}
body.dashboard-v3 .metric-card:nth-child(5){--metric-accent:155,77,255;animation-delay:200ms}
...
body.dashboard-v3 .metric-card::before{
  ...
  inset:0;
  background:
    linear-gradient(125deg,rgba(255,255,255,.085),transparent 30%),
    radial-gradient(circle at 18% -12%,rgba(var(--metric-accent),.22),transparent 36%);
  opacity:.5;
  transform:scale(.97);
  transition:transform var(--at-motion-slow) ease-out,opacity var(--at-motion-base) ease-out;
}
body.dashboard-v3 .metric-card::after{
  ...
  height:1px;
  border-radius:999px;
  background:linear-gradient(90deg,transparent,rgba(var(--metric-accent),.82),transparent);
  opacity:.42;
  transform:scaleX(.72);
  transition:transform var(--at-motion-slow) ease-out,opacity var(--at-motion-base) ease-out;
}
body.dashboard-v3 .metric-card:hover{
  transform:translate3d(0,-3px,0) scale(1.008) !important;
  box-shadow:0 24px 54px rgba(0,2,12,.42),0 0 32px rgba(var(--metric-accent),.13),inset 0 1px rgba(255,255,255,.07) !important;
}
body.dashboard-v3 .metric-card:hover::before{opacity:.88;transform:scale(1)}
body.dashboard-v3 .metric-card:hover::after{opacity:.92;transform:scaleX(1)}
body.dashboard-v3 .metric-card > *{position:relative;z-index:2}
body.dashboard-v3 .metric-icon{
  border-color:rgba(var(--metric-accent),.42);
  background:linear-gradient(145deg,rgba(var(--metric-accent),.22),rgba(var(--metric-accent),.055));
  box-shadow:0 0 0 1px rgba(var(--metric-accent),.06),0 0 24px rgba(var(--metric-accent),.19),inset 0 1px rgba(255,255,255,.12);
  animation:premiumIconBreath 3.6s ease-in-out infinite;
}
...
body.dashboard-v3 .metric-value{
  text-shadow:0 0 28px rgba(var(--metric-accent),.16);
}
body.dashboard-v3 .metric-spark,
body.dashboard-v3 .market-spark,
body.dashboard-v3 .wave-spark{
  filter:drop-shadow(0 0 4px rgba(var(--metric-accent,78,167,255),.68)) drop-shadow(0 0 10px rgba(var(--metric-accent,78,167,255),.22));
  transform-origin:left center;
  animation:premiumSparkReveal .65s var(--at-ease) both;
}
body.dashboard-v3 .metric-spark path,
body.dashboard-v3 .market-spark path,
body.dashboard-v3 .wave-spark path{stroke-width:2.1}
@keyframes premiumSparkReveal{
  from{opacity:0;transform:scaleX(.16) translate3d(-6px,0,0)}
  to{opacity:1;transform:scaleX(1) translate3d(0,0,0)}
}
body.dashboard-v3 .metric-card-brand{
  background:
    radial-gradient(circle at 50% 40%,rgba(116,78,255,.3),transparent 45%),
    radial-gradient(circle at 50% 96%,rgba(45,156,255,.16),transparent 48%),
...
/* Elevated chart cockpit; chart canvas and data remain untouched */
body.dashboard-v3 .chart-shell{
  position:relative;
  padding:16px !important;
  gap:10px;
  border-color:rgba(105,151,225,.21) !important;
  border-radius:16px !important;
  background:
    radial-gradient(ellipse 50% 22% at 24% 0,rgba(60,126,255,.12),transparent 72%),
    radial-gradient(ellipse 34% 20% at 88% 0,rgba(145,70,255,.1),transparent 74%),
    linear-gradient(168deg,rgba(10,21,41,.96),rgba(3,9,21,.975) 54%,rgba(5,11,25,.96)) !important;
  background-image:
    radial-gradient(ellipse 50% 22% at 24% 0,rgba(60,126,255,.12),transparent 72%),
    radial-gradient(ellipse 34% 20% at 88% 0,rgba(145,70,255,.1),transparent 74%),
    linear-gradient(168deg,rgba(10,21,41,.96),rgba(3,9,21,.975) 54%,rgba(5,11,25,.96)) !important;
  box-shadow:var(--at-shadow-float) !important;
  backdrop-filter:blur(var(--at-blur-panel)) saturate(126%);
}
body.dashboard-v3 .chart-shell::before{
  ...
  border-radius:inherit;
  background:
    linear-gradient(125deg,rgba(255,255,255,.055),transparent 24%),
    radial-gradient(circle at 52% -12%,rgba(85,112,255,.2),transparent 32%);
  opacity:.7;
  pointer-events:none;
}
body.dashboard-v3 .chart-shell::after{
  display:block !important;
  content:'';
  position:absolute;
  z-index:1;
...
body.dashboard-v3 .chart-shell > *{position:relative;z-index:2}
...
body.dashboard-v3 .chart-title{
  background:linear-gradient(110deg,#fff,#dce8ff 58%,#9ebcff);
  background-clip:text;
  -webkit-background-clip:text;
  color:transparent !important;
  text-shadow:0 0 28px rgba(89,139,255,.14);
}
body.dashboard-v3 .chart-structure{
  border-color:rgba(119,155,220,.18);
  background:linear-gradient(145deg,rgba(16,29,53,.78),rgba(7,14,29,.72));
  box-shadow:var(--at-shadow-z1);
  backdrop-filter:blur(14px);
}
body.dashboard-v3 .chart-fs-btn,
body.dashboard-v3 .ind-toolbar{
  border-color:rgba(108,150,221,.18) !important;
  background:linear-gradient(180deg,rgba(15,28,51,.84),rgba(6,14,29,.78)) !important;
  box-shadow:var(--at-shadow-z1) !important;
  backdrop-filter:blur(16px) saturate(130%);
}
body.dashboard-v3 .chart-fs-btn,
body.dashboard-v3 .ind-btn{
  transition:transform var(--at-motion-fast) ease-out,opacity var(--at-motion-fast) ease-out !important;
}
body.dashboard-v3 .chart-fs-btn:hover,
body.dashboard-v3 .ind-btn:hover{transform:translate3d(0,-1px,0) scale(1.025) !important;opacity:1}
body.dashboard-v3 .chart-fs-btn:active,
body.dashboard-v3 .ind-btn:active{transform:scale(.97) !important}
body.dashboard-v3 .ind-btn{
  border:1px solid rgba(105,147,218,.1) !important;
  background:linear-gradient(180deg,rgba(18,34,60,.68),rgba(8,17,33,.62)) !important;
  opacity:.7;
}
body.dashboard-v3 .ind-btn.on,
body.dashboard-v3 .ind-btn.active{
  opacity:1;
  border-color:rgba(78,167,255,.32) !important;
  box-shadow:0 0 18px rgba(53,109,255,.14),inset 0 1px rgba(255,255,255,.06) !important;
}
body.dashboard-v3 #chart-container{
  border-color:rgba(92,139,218,.18) !important;
  border-radius:12px;
  background:
    linear-gradient(rgba(53,109,255,.025) 1px,transparent 1px),
    linear-gradient(90deg,rgba(53,109,255,.025) 1px,transparent 1px),
    radial-gradient(ellipse at 50% 0,rgba(55,104,225,.08),transparent 62%),
    linear-gradient(180deg,rgba(2,8,19,.98),rgba(1,5,13,.99)) !important;
  background-size:54px 54px,54px 54px,100% 100%,100% 100% !important;
  box-shadow:inset 0 1px rgba(255,255,255,.025),inset 0 0 80px rgba(9,31,70,.14),0 20px 44px rgba(0,2,10,.34) !important;
}

/* Market telemetry below the chart */
body.dashboard-v3 .market-grid{gap:10px;margin-top:4px !important}
body.dashboard-v3 .market-card,
body.dashboard-v3 .wave-card{
  --metric-accent:78,167,255;
  position:relative;
  overflow:hidden;
  border-color:rgba(var(--metric-accent),.13) !important;
  background:
    radial-gradient(circle at 100% 110%,rgba(var(--metric-accent),.12),transparent 48%),
    linear-gradient(145deg,rgba(14,28,51,.86),rgba(5,12,25,.9)) !important;
  box-shadow:var(--at-shadow-z1) !important;
  transition:transform var(--at-motion-base) ease-out,opacity var(--at-motion-fast) ease-out !important;
}
body.dashboard-v3 .market-card:nth-child(2){--metric-accent:155,77,255}
body.dashboard-v3 .market-card:nth-child(3){--metric-accent:176,73,255}
body.dashboard-v3 .market-card:nth-child(4){--metric-accent:90,89,255}
body.dashboard-v3 .market-card:nth-child(5){--metric-accent:32,227,178}
body.dashboard-v3 .market-card::before,
body.dashboard-v3 .wave-card::before{
  content:'';
  position:absolute;
  inset:0;
  background:linear-gradient(128deg,rgba(255,255,255,.05),transparent 34%);
  opacity:.45;
  transform:scale(.98);
  transition:transform var(--at-motion-slow) ease-out,opacity var(--at-motion-base) ease-out;
  pointer-events:none;
}
body.dashboard-v3 .market-card:hover,
body.dashboard-v3 .wave-card:hover{transform:translate3d(0,-2px,0) scale(1.006) !important}
body.dashboard-v3 .market-card:hover::before,
body.dashboard-v3 .wave-card:hover::before{opacity:.9;transform:scale(1)}
body.dashboard-v3 .market-card > *,body.dashboard-v3 .wave-card > *{position:relative;z-index:1}
body.dashboard-v3 .market-value,
body.dashboard-v3 .wave-value{text-shadow:0 0 22px rgba(var(--metric-accent),.16)}
body.dashboard-v3 .sentiment-gauge{
  filter:drop-shadow(0 0 8px rgba(32,227,178,.34));
  animation:premiumGaugeEnter .62s var(--at-ease) both;
}
@keyframes premiumGaugeEnter{
  from{opacity:0;transform:rotate(-90deg) scale(.72)}
  to{opacity:1;transform:rotate(-90deg) scale(1)}
}
body.dashboard-v3 .wave-row{gap:10px}
body.dashboard-v3 .wave-card{min-height:92px}
body.dashboard-v3 .wave-card:nth-child(1){--metric-accent:32,227,178}
body.dashboard-v3 .wave-card:nth-child(2){--metric-accent:78,167,255}
...
body.dashboard-v3 .terminal-card{
  ...
  border-radius:12px !important;
  background:
    radial-gradient(circle at 92% 0,rgba(73,119,255,.08),transparent 32%),
    linear-gradient(155deg,rgba(14,28,51,.82),rgba(5,12,25,.87)) !important;
  background-image:
    radial-gradient(circle at 92% 0,rgba(73,119,255,.08),transparent 32%),
    linear-gradient(155deg,rgba(14,28,51,.82),rgba(5,12,25,.87)) !important;
  box-shadow:var(--at-shadow-z2) !important;
...
body.dashboard-v3 .terminal-card:first-child{border-radius:12px !important}
body.dashboard-v3 .terminal-card-hdr{
  background:
    linear-gradient(180deg,rgba(23,40,69,.72),rgba(9,18,35,.54)),
    linear-gradient(90deg,rgba(78,167,255,.06),transparent) !important;
  box-shadow:inset 0 1px rgba(255,255,255,.035);
}
body.dashboard-v3 .exec-pill{
  position:relative;
  box-shadow:0 0 18px rgba(32,227,178,.12),inset 0 1px rgba(255,255,255,.05);
}
body.dashboard-v3 .exec-pill::before{
  content:'';
  width:5px;
  height:5px;
  margin-right:5px;
...
body.dashboard-v3 .exec-btn,
body.dashboard-v3 .exec-mode-btn{
  background:linear-gradient(180deg,rgba(20,37,65,.78),rgba(8,17,33,.74)) !important;
  box-shadow:inset 0 1px rgba(255,255,255,.035),0 8px 18px rgba(0,2,10,.18) !important;
  transition:transform var(--at-motion-fast) ease-out,opacity var(--at-motion-fast) ease-out !important;
}
body.dashboard-v3 .exec-btn:hover,
body.dashboard-v3 .exec-mode-btn:hover{transform:translate3d(0,-2px,0) scale(1.012) !important}
body.dashboard-v3 .exec-btn:active,
body.dashboard-v3 .exec-mode-btn:active{transform:scale(.975) !important}
body.dashboard-v3 .exec-btn.active{
  background:
    radial-gradient(circle at 50% 0,rgba(32,227,178,.22),transparent 68%),
    linear-gradient(180deg,rgba(18,112,99,.52),rgba(5,40,40,.62)) !important;
  box-shadow:var(--at-glow-teal) !important;
}
body.dashboard-v3 .exec-exposure{
  position:relative;
  overflow:hidden;
  background:
    radial-gradient(circle at 0 0,rgba(245,185,77,.13),transparent 40%),
    linear-gradient(145deg,rgba(42,31,14,.56),rgba(7,15,29,.88)) !important;
  box-shadow:var(--at-shadow-z1),inset 0 1px rgba(255,255,255,.045) !important;
}
body.dashboard-v3 .exec-pnl-panel{
  position:relative;
  background:
    radial-gradient(circle at 100% 0,rgba(32,227,178,.09),transparent 40%),
    linear-gradient(145deg,rgba(11,26,43,.8),rgba(4,12,24,.86)) !important;
  box-shadow:var(--at-shadow-z1),inset 0 1px rgba(255,255,255,.035) !important;
}
body.dashboard-v3 .details-card{padding-bottom:8px}
body.dashboard-v3 .pnl-hero{
  margin:8px 8px 0;
  border:1px solid rgba(89,144,227,.14);
  border-radius:11px;
  background:
    radial-gradient(circle at 88% 0,rgba(32,227,178,.1),transparent 38%),
    linear-gradient(145deg,rgba(14,28,50,.76),rgba(5,13,27,.8));
  box-shadow:var(--at-shadow-z1);
}
body.dashboard-v3 .details-card .sec{
  margin:8px 8px 0;
  padding:11px 12px;
  border:1px solid rgba(99,143,215,.105);
  border-radius:10px;
  background:linear-gradient(145deg,rgba(14,27,48,.62),rgba(5,13,26,.66));
  box-shadow:inset 0 1px rgba(255,255,255,.025),0 8px 18px rgba(0,2,10,.13);
}
...
body.dashboard-v3 .stat-c{
  ...
  overflow:hidden;
  background:
    linear-gradient(135deg,rgba(20,37,62,.68),rgba(7,16,31,.72)) !important;
  box-shadow:inset 0 1px rgba(255,255,255,.03),0 7px 16px rgba(0,2,10,.12) !important;
  transition:transform var(--at-motion-fast) ease-out,opacity var(--at-motion-fast) ease-out !important;
}
body.dashboard-v3 .lv:hover,
body.dashboard-v3 .stat-c:hover{transform:translate3d(0,-1px,0) scale(1.008) !important}
body.dashboard-v3 .lv-entry{box-shadow:inset 2px 0 rgba(78,167,255,.7),0 7px 16px rgba(0,2,10,.12) !important}
body.dashboard-v3 .lv-sl{box-shadow:inset 2px 0 rgba(255,79,109,.7),0 7px 16px rgba(0,2,10,.12) !important}
body.dashboard-v3 .lv-tp{box-shadow:inset 2px 0 rgba(32,227,178,.7),0 7px 16px rgba(0,2,10,.12) !important}

/* Premium loading state and skeleton pass */
body.dashboard-v3 .page-loader{
  background:
    radial-gradient(circle at 50% 42%,rgba(72,86,255,.2),transparent 28%),
    rgba(1,5,14,.88);
  backdrop-filter:blur(24px) saturate(130%);
}
body.dashboard-v3 .loading-shell{
  min-width:min(86vw,340px);
  padding:26px;
  border:1px solid rgba(110,150,225,.2);
  border-radius:16px;
  background:
    radial-gradient(circle at 50% 0,rgba(83,112,255,.16),transparent 44%),
    linear-gradient(155deg,rgba(16,29,54,.94),rgba(5,12,27,.96));
  box-shadow:var(--at-shadow-float);
...
body.dashboard-v3 .loading-bars{
  display:flex !important;
  position:absolute;
  inset:8px 12px;
  align-items:flex-end;
...
body.dashboard-v3 .loading-bars span{
  width:6px;
  height:36px;
  border-radius:999px;
  background:linear-gradient(180deg,#7ab8ff,#7257ff);
  box-shadow:0 0 16px rgba(83,112,255,.42);
  transform-origin:center bottom;
  animation:loadingBarsPremium 1.35s ease-in-out infinite;
}
body.dashboard-v3 .loading-bars span:nth-child(2){animation-delay:100ms}
body.dashboard-v3 .loading-bars span:nth-child(3){animation-delay:200ms}
body.dashboard-v3 .loading-bars span:nth-child(4){animation-delay:300ms}
body.dashboard-v3 .loading-bars span:nth-child(5){animation-delay:400ms}
@keyframes loadingBarsPremium{
  0%,100%{transform:scaleY(.26);opacity:.38}
  50%{transform:scaleY(1);opacity:1}
}
body.dashboard-v3 .loading-gridline{
  display:block !important;
  position:absolute;
  left:12%;right:12%;bottom:7px;height:1px;
  background:var(--at-highlight);
  opacity:.5;
}
body.dashboard-v3 .loading-pulses{display:none !important}
body.dashboard-v3:not(.page-ready) :is(.metric-card,.market-card,.wave-card)::after{
  content:'';
  position:absolute;
  inset:-20% auto -20% -32%;
  width:26%;
  height:auto;
  background:linear-gradient(90deg,transparent,rgba(155,194,255,.16),transparent);
  opacity:0;
...
@keyframes premiumSkeletonPass{
  ...
  20%{opacity:1}
  100%{transform:translate3d(580%,0,0) skewX(-16deg);opacity:0}
}

/* Keyboard focus is luminous without moving layout. */
body.dashboard-v3 :is(button,a,[tabindex]):focus-visible{
  outline:0;
  box-shadow:0 0 0 2px rgba(4,10,22,.95),0 0 0 4px rgba(78,167,255,.72),0 0 24px rgba(78,167,255,.28) !important;
}

/* Responsive depth: retain composition without expensive effects on mobile. */
@media (max-width:1480px){
...
@media (max-width:1024px){
  ...
  body.dashboard-v3 .sb{top:78px;height:calc(100dvh - 90px) !important;max-height:calc(100dvh - 90px) !important}
  body.dashboard-v3 .wl{height:auto !important;max-height:none !important}
}
@media (max-width:840px){
  body.dashboard-v3{--terminal-header:58px}
  body.dashboard-v3 .nav-shell{top:6px;width:calc(100% - 16px);height:50px;min-height:50px;margin:6px 8px 0;border-radius:12px !important}
  body.dashboard-v3 .layout{padding:14px 8px 74px !important}
  body.dashboard-v3 .wl,body.dashboard-v3 .sb{top:62px;height:auto !important;max-height:none !important}
  body.dashboard-v3 .metric-card{min-height:112px}
  body.dashboard-v3 .chart-shell{padding:10px !important}
}

html[data-theme='light'] body.dashboard-v3{
  background:
    radial-gradient(circle at 12% -12%,rgba(53,109,255,.12),transparent 29%),
    linear-gradient(180deg,#eef4fb,#f7f9fc 52%,#edf2f8) !important;
}
html[data-theme='light'] body.dashboard-v3 .nav-shell{
  background:rgba(255,255,255,.86);
  box-shadow:0 8px 30px rgba(39,61,92,.09),inset 0 -1px rgba(53,109,255,.04);
}
html[data-theme='light'] body.dashboard-v3 .nav-market,
html[data-theme='light'] body.dashboard-v3 .theme-toggle,
html[data-theme='light'] body.dashboard-v3 .nav-user,
html[data-theme='light'] body.dashboard-v3 .nav-link-ghost{
  background:rgba(247,250,254,.9);
}
html[data-theme='light'] body.dashboard-v3 .wl,
html[data-theme='light'] body.dashboard-v3 .sb{
  background:linear-gradient(180deg,rgba(255,255,255,.94),rgba(245,249,254,.96)) !important;
}
html[data-theme='light'] body.dashboard-v3 .workspace-card{
  background:
    radial-gradient(circle at 8% 0,rgba(78,167,255,.11),transparent 34%),
    linear-gradient(155deg,rgba(255,255,255,.98),rgba(241,247,254,.96)) !important;
}
html[data-theme='light'] body.dashboard-v3 .telemetry-card,
html[data-theme='light'] body.dashboard-v3 .terminal-card-hdr{
  background:rgba(240,246,253,.76) !important;
}
html[data-theme='light'] body.dashboard-v3 .wl-item,
html[data-theme='light'] body.dashboard-v3 .lv,
html[data-theme='light'] body.dashboard-v3 .stat-c{
  background:rgba(238,245,253,.72) !important;
}
html[data-theme='light'] body.dashboard-v3 .wl-item.active{
  background:linear-gradient(90deg,rgba(53,109,255,.11),rgba(242,247,253,.78)) !important;
}
html[data-theme='light'] body.dashboard-v3 .chart-shell{
  background:
    radial-gradient(circle at 20% 0,rgba(53,109,255,.055),transparent 31%),
    linear-gradient(180deg,rgba(255,255,255,.98),rgba(242,247,253,.98)) !important;
  background-image:
    radial-gradient(circle at 20% 0,rgba(53,109,255,.055),transparent 31%),
    linear-gradient(180deg,rgba(255,255,255,.98),rgba(242,247,253,.98)) !important;
}
html[data-theme='light'] body.dashboard-v3 .chart-structure,
html[data-theme='light'] body.dashboard-v3 .chart-fs-btn,
html[data-theme='light'] body.dashboard-v3 .ind-toolbar,
html[data-theme='light'] body.dashboard-v3 .ind-btn{
  background:rgba(244,248,253,.9) !important;
}
html[data-theme='light'] body.dashboard-v3 #chart-container,
html[data-theme='light'] body.dashboard-v3 .sub-panel{
  background:linear-gradient(180deg,#fff,#f7f9fc) !important;
}
html[data-theme='light'] body.dashboard-v3 .exec-btn,
html[data-theme='light'] body.dashboard-v3 .exec-mode-btn,
html[data-theme='light'] body.dashboard-v3 .exec-pnl-panel{
  background:rgba(240,246,253,.84) !important;
}
html[data-theme='light'] body.dashboard-v3 .exec-exposure{
  background:linear-gradient(145deg,rgba(255,248,229,.72),rgba(247,250,254,.9));
}
html[data-theme='light'] body.dashboard-v3 .app-chrome{
  opacity:.52;
  background:
    radial-gradient(circle at 22% 16%,rgba(65,111,255,.13),transparent 22%),
    radial-gradient(circle at 78% 22%,rgba(148,70,255,.08),transparent 24%);
}
html[data-theme='light'] body.dashboard-v3 .app-vignette{
  background:radial-gradient(ellipse at center,transparent 44%,rgba(67,91,128,.12) 100%);
}
html[data-theme='light'] body.dashboard-v3 .nav-logo{
  background:linear-gradient(135deg,rgba(72,118,255,.1),rgba(132,72,255,.04));
  box-shadow:inset 0 1px #fff,0 10px 24px rgba(39,61,92,.08);
}
html[data-theme='light'] body.dashboard-v3 .metric-card,
html[data-theme='light'] body.dashboard-v3 .market-card,
html[data-theme='light'] body.dashboard-v3 .wave-card{
  background:
    radial-gradient(circle at 8% 0,rgba(var(--metric-accent),.11),transparent 38%),
    radial-gradient(circle at 94% 106%,rgba(var(--metric-accent),.07),transparent 42%),
    linear-gradient(145deg,rgba(255,255,255,.96),rgba(240,246,253,.95)) !important;
  box-shadow:var(--at-shadow-z2) !important;
}
html[data-theme='light'] body.dashboard-v3 .metric-card-brand{
  background:
    radial-gradient(circle at 50% 40%,rgba(116,78,255,.18),transparent 45%),
    linear-gradient(145deg,rgba(247,244,255,.98),rgba(232,240,253,.97)) !important;
}
html[data-theme='light'] body.dashboard-v3 .chart-title{
  background:linear-gradient(110deg,#111b2c,#294e86 62%,#536d9c);
  background-clip:text;
  -webkit-background-clip:text;
}
html[data-theme='light'] body.dashboard-v3 .terminal-card{
  background:
    radial-gradient(circle at 92% 0,rgba(73,119,255,.08),transparent 32%),
    linear-gradient(155deg,rgba(255,255,255,.94),rgba(240,246,253,.94)) !important;
  background-image:
    radial-gradient(circle at 92% 0,rgba(73,119,255,.08),transparent 32%),
    linear-gradient(155deg,rgba(255,255,255,.94),rgba(240,246,253,.94)) !important;
}
html[data-theme='light'] body.dashboard-v3 .details-card .sec,
html[data-theme='light'] body.dashboard-v3 .pnl-hero,
html[data-theme='light'] body.dashboard-v3 .lv,
html[data-theme='light'] body.dashboard-v3 .stat-c{
  background:linear-gradient(145deg,rgba(255,255,255,.9),rgba(235,243,252,.86)) !important;
}
html[data-theme='light'] body.dashboard-v3 .exec-pnl-panel{
  background:
    radial-gradient(circle at 100% 0,rgba(32,227,178,.08),transparent 40%),
    linear-gradient(145deg,rgba(255,255,255,.9),rgba(235,245,250,.86)) !important;
}
@media (prefers-reduced-motion:reduce){
  html[data-theme] body.dashboard-v3 *,
  html[data-theme] body.dashboard-v3 *::before,
  html[data-theme] body.dashboard-v3 *::after{
